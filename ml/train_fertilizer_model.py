...
Trains a RandomForestClassifier on fertilizer.csv.
Saves model to ml/models/fertilizer_model.pkl and feature importances to ml/models/fertilizer_feature_importance.png.
"""
...
def train_fertilizer_model():
    ...
    print(f"[Fertilizer Model] Loading dataset from {data_path}...")
    df = pd.read_csv(data_path)
    
    cat_features = ['crop_type']
    num_features = ['soil_ph', 'nitrogen', 'phosphorus', 'potassium', 'moisture_pct']
    ...
    print("\n" + "="*50)
    print("Fertilizer Recommendation Evaluation")
    print("="*50)
    ...
    plt.barh(range(len(sorted_features)), sorted_importances[::-1], color='#0284c7', align='center')
    ...
    plt.title('Fertilizer Recommendation - Feature Importances', fontsize=13, fontweight='bold', pad=15)
    plt.grid(axis='x', linestyle='--', alpha=0.6)
    plt.tight_layout()
    
    plot_path = os.path.join(models_dir, 'fertilizer_feature_importance.png')
